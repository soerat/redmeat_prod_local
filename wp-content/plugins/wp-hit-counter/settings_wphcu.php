...
function wphcu_init() {
	register_setting('wp-hit-counter-group', 'wphcu_exclude_ips',  'iplist');
    register_setting('wp-hit-counter-group', 'wphcu_display_footer', 'intval');
    register_setting('wp-hit-counter-group', 'wphcu_display_credit', 'intval');
    register_setting('wp-hit-counter-group', 'wphcu_style');
    register_setting('wp-hit-counter-group', 'wphcu_data', 'intval');
    register_setting('wp-hit-counter-group', 'wphcu_align');
    register_setting('wp-hit-counter-group', 'wphcu_count_only_unique', 'intval');}
...
function wphcu_menu() {
  add_options_page('WP Hit Counter Options', 'WP Hit Counter', 'administrator', 'wp-hit-counter', 'wphcu_options');
}
...
function wphcu_options() {
    ...
    <h2>WP Hit Counter Settings</h2>

    <form method="post" action="options.php">
        <?php settings_fields( 'wp-hit-counter-group' ); ?>
        <?php
            $data = file_array(WP_CONTENT_DIR . '/plugins/wp-hit-counter/designs/');
    ...
    <?php
                foreach ($style as $name) {
                	...
                	<tr>
                		...
                		<img src='<?php echo WP_PLUGIN_URL?>/wp-hit-counter/designs/<?php echo $style_name . '/' . $name . '/'; ?>1.gif'>
                		<img src='<?php echo WP_PLUGIN_URL?>/wp-hit-counter/designs/<?php echo $style_name . '/' . $name . '/'; ?>2.gif'>
                		<img src='<?php echo WP_PLUGIN_URL?>/wp-hit-counter/designs/<?php echo $style_name . '/' . $name . '/'; ?>3.gif'>
                		</td>
                	</tr>
                    <?php
                }
