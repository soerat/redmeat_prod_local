...
*/
	...
	savonaLivePreview( 'colors_header_bg', function( val ) {
		$('.entry-header').css( 'background-color', val );
	});

	// Accent Color
	savonaLivePreview( 'colors_content_accent', function( val ) {
		var selectors = '\
			#page-content h1 a,\
		...
		#page-content a:hover {\
			color: '+ savonaHex2Rgba( val, 0.8 ) +';\
		}';	

		savonaStyle( 'colors_content_accent', css );
	});
...
*/
	...
	savonaLivePreview( 'title_tagline_logo_width', function( val ) {
...
*/
	// Previewer
	function savonaLivePreview( control, func ) {
		wp.customize( 'savona_options['+ control +']', function( value ) {
			value.bind( function( val ) {
				func( val );
			} );
		} );
	...
	function savonaHex2Rgba( hex, opacity ) {
